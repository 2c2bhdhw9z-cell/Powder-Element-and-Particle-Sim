 ...
 *     issuer / upstream login page. The popup never paints the app.
 *   Phase 2 (`?done=1`): after the issuer round-trip, emit a tiny HTML page that
...
export async function handleAuthPopupRequest(request: Request): Promise<Response> {
  ...
  try {
    const apiRes = await auth.api.signInWithOAuth2({
      ...
      // redirect_uri for the dynamic embedded-preview origin.
...
<script type="application/json" id="crucible-auth-popup-msg">${payload}</script>
<script>
(function () {
  var el = document.getElementById("crucible-auth-popup-msg");
  var msg = { source: "crucible-auth-popup", token: null };
