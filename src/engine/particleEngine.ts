...
export class ParticleEngine {
  ...
  public runAutoFix(): { logs: string[] } {
    const logs: string[] = [];
    logs.push('Initiating Particle Simulator Diagnostics Check...');
    ...
    if (diag.nanCount > 0) {
      ...
      logs.push(`Auto-Fix [1/3]: Purged ${res.purged} corrupt/NaN particles.`);
    }

    if (diag.outOfBoundsCount > 0) {
      const res = this.wrapOrTrimOutOfBounds();
      logs.push(`Auto-Fix [2/3]: Re-centered ${res.trimmed} out-of-bounds particles.`);
    }

    if (diag.extremeVelocityCount > 0) {
      const res = this.clampVelocities();
      logs.push(`Auto-Fix [3/3]: Clamped velocities for ${res.clamped} hyper-fast particles.`);
    }

    this.reallocateBuffers();
    logs.push('Auto-Fix Sequence Completed. Particle Engine restored to healthy state.');
    return { logs };
  }
}
